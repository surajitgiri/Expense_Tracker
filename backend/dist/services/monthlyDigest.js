...
exports.generateAndSendDigestForUser = generateAndSendDigestForUser;
exports.sendMonthlyDigestsToAllUsers = sendMonthlyDigestsToAllUsers;
 ...
 */
async function generateAndSendDigestForUser(userId, referenceDate = new Date()) {
    try {
        ...
        const biggestCategory = topCategory && totalSpent > 0
            ? {
        ...
        await (0, mail_1.sendMonthlyDigestEmail)(user.email, user.name || "User", digestData);
 ...
 */
async function sendMonthlyDigestsToAllUsers(referenceDate = new Date()) {
    ...
    console.log(`✅ Monthly Financial Digest completed: ${successCount} sent, ${failureCount} failed.`);
